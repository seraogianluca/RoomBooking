...
public final class RoomBookingCLI {
	...
	/*
	 * private static void ident() throws UserNotExistException { String email =
	 * null; boolean isValid = false; boolean isValid2 = false; String value;
	 * System.out.println(BLUE + version + WHITE);
	 * 
	 * while (!isValid) { try { while (!isValid2) { System.out
	 * .print("\nAre you a teacher or a student?\n" + "\nT - Teacher" +
	 * "\nS - Student\n" + "\n>");
	 * 
	 * value = input.next();
	 * 
	 * if (value.toLowerCase().equals("t")) { isTeacher = true; isValid2 = true; }
	 * else if (value.toLowerCase().equals("s")) { isTeacher = false; isValid2 =
	 * true; } else { System.out.print(YELLOW + "Insert a correct value\n" + WHITE);
	 * isValid2 = false; } }
	 * 
	 * System.out.print("\nInsert your Email > "); email = input.next().toString();
	 * user = database.authenticate(email, isTeacher); isValid = true; } catch
	 * (UserNotExistException uex) { System.out.println(RED + uex.getMessage() +
	 * WHITE); isValid = false; isValid2 = false; } } }
	 * 
	 * private static String getCommand() { String command; boolean isValid = false;
	 * 
	 * System.out .println("\n1 - Book a Room." + "\n2 - Delete a booking." +
	 * "\n3 - Update a booking." + "\n4 - Close.");
	 * 
	 * while (!isValid) { System.out.print("\nChoose an action > "); command =
	 * input.next();
	 * 
	 * if (!command.equals("4") && !command.equals("3") && !command.equals("2") &&
	 * !command.equals("1")) { System.out.println(YELLOW +
	 * "\nPlease insert a valid command." + WHITE); } else { isValid = true; return
	 * command; } }
	 * 
	 * return null; }
	 * 
	 * private static String setSchedule() { String requestedSchedule = null;
	 * boolean isValid = false;
	 * 
	 * System.out.println("\n[M] - Morning." + "\n[A] - Afternoon.");
	 * 
	 * while (!isValid) { System.out.print("\nChoose a schedule > ");
	 * requestedSchedule = input.next(); requestedSchedule =
	 * requestedSchedule.toLowerCase(Locale.ENGLISH);
	 * 
	 ...
	 * WHITE); } else { database.setBooking(user, Long.parseLong(requestedRoom),
	 * requestedSchedule); System.out.println(GREEN + "\nRoom succesfully booked." +
	 * WHITE); } } }
	 * 
	 * private static void deleteBooking() {
	 * 
	 * String requestedRoom = null; Collection<? extends Room> bookedRooms; boolean
	 * isValid = false; long bookingId = -1;
	 * 
	 ...
	 * 
	 * if (i.getId() == Long.parseLong(requestedRoom)) { bookingId = i.getId();
	 * isValid = true; break; } } }
	 * 
	 * if (!isValid) { System.out.println(YELLOW + "\nPlease insert a valid room." +
	 * WHITE); } else { database.deleteBooking(user, bookingId);
	 * System.out.println(GREEN + "\nBooking succesfully deleted." + WHITE); } } }
	 * 
	 * private static void updateBooking() {
	 * 
	 * String oldRoom = null; String requestedSchedule = null; String requestedRoom
	 * = null; Collection<? extends Room> availableRooms; Collection<? extends Room>
	 * bookedRooms; boolean isValid = false; String oldbookingId = null; long
	 * oldRoomId = -1;
	 * 
	 * bookedRooms = database.getBooked(user);
	 * 
	 ...
	 * 
	 * for (Room iteration : bookedRooms) { Classroom c = (Classroom) iteration;
	 * Collection<ClassroomBooking> collection =
	 * c.getBookedByTeacherId(user.getId()); for (ClassroomBooking iterator :
	 ...
	 * 
	 * if (!isValid) { System.out.println(YELLOW + "\nPlease insert a valid room." +
	 * WHITE); } else { System.out.print("\nChoose the new schedule: ");
	 * requestedSchedule = setSchedule(); } } } else { while (!isValid) {
	 * System.out.print("\nChoose the room you want to change by ID > "); oldRoom =
	 * input.next(); oldRoomId = Long.parseLong(oldRoom); for (Room i : bookedRooms)
	 * { if (i.getId() == oldRoomId) { isValid = true; oldbookingId = "1"; // for
	 * the student this value is not used after break; } }
	 * 
	 * if (!isValid) { System.out.println(YELLOW + "\nPlease insert a valid room." +
	 * WHITE); } } }
	 * 
	 ...
	 * 
	 * System.out.print("\nChoose a room by ID > "); requestedRoom = input.next();
	 * for (Room i : availableRooms) { if (i.getId() ==
	 * Long.parseLong(requestedRoom)) { // idroom isValid = true; break; } }
	 * 
	 * if (!isValid) { System.out.println(YELLOW + "\nPlease insert a valid room." +
	 * WHITE); } else { database.updateBooking(user, oldRoomId,
	 * Long.parseLong(requestedRoom), Long.parseLong(oldbookingId),
	 * requestedSchedule); System.out.println(GREEN +
	...
	public static void main(String[] args) {
		...
		try {
			...
			database.initializeAvailable(user2);
			database.initializeBooked(user2);
			
			bookings.clear();
			bookings = database.getBooked(user2.getRole());
			System.out.println("Teacher booking:");
			System.out.println(bookings.toString());

			availables.clear();
			availables = database.getAvailable("m", user2.getRole());
